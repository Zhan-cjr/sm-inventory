...
export const POSTransaction = ({ branchId, branchName, orgName, authToken, userName, onLogout }) => {
  ...
  const [heldTransactions, setHeldTransactions] = useState(JSON.parse(localStorage.getItem('pos_held_transactions') || '[]'));
  ...
  useEffect(() => {
    const handleShortcuts = (e) => {
      ...
      const setting = posSettings.find(s => s.shortcut_key.toLowerCase() === e.key.toLowerCase());
      if (!setting) return;
  ...
  }, [posSettings, items, isSubtotalMode, receivedAmount]);
  ...
  const checkActiveShift = (terminalId) => {
    ...
    fetch(`/api/v1/shifts/active?terminal_id=${terminalId}`, {
      ...
      .catch(err => console.error('Failed to check shift:', err))
  ...
  const handleOpenShift = () => {
    ...
    fetch('/api/v1/shifts/open', {
      ...
      .catch(err => setAlertMsg({ text: 'Terjadi kesalahan sistem.', type: 'error' }));
  };
  ...
  const handleCloseShift = () => {
    ...
    fetch('/api/v1/shifts/close', {
      ...
      .catch(err => {
        ...
        setAlertMsg({ text: 'Terjadi kesalahan sistem saat menutup shift.', type: 'error' });
      });
  };
  ...
  const handleManualDiscountItem = (type) => {
    const val = parseFloat(barcodeInput.current.value);
    if (isNaN(val)) {
      setAlertMsg({ text: 'Masukkan angka nominal diskon dulu!', type: 'error' });
      setTimeout(() => setAlertMsg(null), 2000);
      return;
    }
    setQueuedDiscount({ type, value: val });
    barcodeInput.current.value = '';
    barcodeInput.current.focus();
    setAlertMsg({ text: `Diskon ${type === 'PERCENT' ? val + '%' : 'Rp ' + val} disiapkan. Silakan scan barang.`, type: 'info', persist: true });
  };

  const handleManualTotalDiscount = (type) => {
    const val = parseFloat(barcodeInput.current.value);
    if (isNaN(val)) {
      setAlertMsg({ text: 'Masukkan angka nominal diskon total dulu!', type: 'error' });
      setTimeout(() => setAlertMsg(null), 2000);
      return;
    }
    let discount = 0;
  ...
  const processTransaction = async (method = paymentMethod, bankId = selectedBank?.id, overrideReceived = null) => {
    ...
    try {
      ...
      const transaction = {
        ...
        discountAmount: totalDiscount + manualTotalDiscount,
      ...
      <div className="pos-main-layout">
                ...
                {items.length === 0 ? (
                  ...
                  items.map((item, index) => (
                    <tr key={item.productId} className={`cart-row fade-in ${item.productId === lastScannedProductId ? 'highlight-row' : ''}`}>
                      ...
                      <td>
                        <button className="btn-remove-item" onClick={() => removeItem(item.productId)}><Trash2 size={16} /></button>
                      </td>
                    </tr>
                  ))
                )}
        ...
        <aside className="pos-functions-sidebar" style={{ padding: '1.5rem', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          ...
          <div className="function-grid">
            <button className="func-btn secondary"><Unlock size={18} /><span>Sign On/Off</span></button>
            <button className="func-btn discount" onClick={() => handleManualDiscountItem('NOMINAL')}><Tag size={18} /><span>Disc Item Rp</span><span className="key-hint">F1</span></button>
            <button className="func-btn discount" onClick={() => handleManualDiscountItem('PERCENT')}><Tag size={18} /><span>Disc Item %</span><span className="key-hint">F2</span></button>
            <button className="func-btn primary"><Package size={18} /><span>Qty</span></button>
            <button className="func-btn secondary"><RotateCcw size={18} /><span>Retur</span></button>
            <button className="func-btn discount" onClick={() => handleManualTotalDiscount('NOMINAL')}><Tag size={18} /><span>Disc Total Rp</span><span className="key-hint">F3</span></button>
            <button className="func-btn discount" onClick={() => handleManualTotalDiscount('PERCENT')}><Tag size={18} /><span>Disc Total %</span><span className="key-hint">F4</span></button>
            ...
            <button className="func-btn action" onClick={handleHoldTransaction}><Lock size={18} /><span>Hold</span></button>
            <button className="func-btn action" onClick={() => setIsRecallModalOpen(true)}><History size={18} /><span>Recall</span></button>
            ...
            <button className="func-btn danger" onClick={() => updateQuantity(items[items.length-1]?.productId, 0)}><Eraser size={18} /><span>Void Item</span><span className="key-hint">Del</span></button>
            <button className="func-btn danger" onClick={() => setItems([])}><Trash2 size={18} /><span>Void All</span><span className="key-hint">Esc</span></button>
            <button className="func-btn secondary" onClick={() => setIsTerminalModalOpen(true)}><Settings size={18} /><span>Menu</span></button>
          </div>

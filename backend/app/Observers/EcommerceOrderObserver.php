            ...
            switch ($status) {
                ...
                case 'CANCELLED':
                    ...
                    if ($order->branch_id) {
                        foreach ($order->items as $item) {
                            ...
                            if ($stock) {
                                ...
                                $stock->notes = 'Pembatalan E-Commerce: ' . $order->customer_name;
                    ...
                    if ($customer) {
                        ...
                        if ($earnedPoints > 0) {
                            $customer->deductPoints($earnedPoints, 'CANCEL', $order->id, "Pembatalan E-Commerce (Batal Poin Belanja): #{$orderId}");
                        }

                        if ($order->points_redeemed > 0) {
                            $customer->addPoints($order->points_redeemed, 'CANCEL', $order->id, "Pembatalan E-Commerce (Kembali Poin Ditukar): #{$orderId}");
                        }
                    }
                    break;
            }

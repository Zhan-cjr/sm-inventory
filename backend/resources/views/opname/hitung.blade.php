...
(function() {
    ...
    // ─── Enter key navigation between qty inputs ───
    ...
    document.getElementById('count-form').addEventListener('submit', function(e) {
        ...
        allInputs.forEach(inp => {
            if (inp.closest('.product-item').style.display !== 'none' && inp.value === '') {
                hasEmpty = true;
            }
        });
        if (hasEmpty && !confirm('Ada produk yang belum diisi qty-nya. Lanjutkan submit?')) {
    ...
    function findAndFocusByBarcode(code) {
        ...
        if (found) {
            ...
            status.textContent = `❌ Produk dengan barcode "${code}" tidak ditemukan di rak ini.`;
        }
    }

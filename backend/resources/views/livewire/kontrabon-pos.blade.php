<div class="h-full flex flex-col bg-white dark:bg-gray-900 rounded-xl shadow-sm border border-gray-200 dark:border-gray-800 overflow-hidden font-sans text-sm">
    ...
    <div style="padding: 1rem; border-bottom: 1px solid #e5e7eb;" class="dark:border-gray-700 bg-gray-50 dark:bg-gray-800">
        <div class="grid-layout">
            ...
            <!-- Kontrabon Panel -->
            <div>
                <div style="font-weight: 600; font-size: 1rem; margin-bottom: 0.5rem;" class="text-gray-900 dark:text-gray-200">Info Kontrabon</div>
                <table style="width: 100%; border-collapse: separate; border-spacing: 0 0.5rem;">
                    <tr>
                        <td style="width: 30%;" class="pos-label">No Kontrabon</td>
                        <td><input type="text" class="pos-input" style="background-color: #e5e7eb; cursor: not-allowed;" readonly wire:model="kontrabon_number"></td>
                    </tr>
                    <tr>
            ...
            <div>
                <div style="font-weight: 600; font-size: 1rem; margin-bottom: 0.5rem;" class="text-gray-900 dark:text-gray-200">Detail Supplier</div>
                <table style="width: 100%; border-collapse: separate; border-spacing: 0 0.5rem;">
                    <tr>
                        ...
                        <td>
                            <div x-data="{ 
                                ...
                                <div x-show="open" @click.away="open = false" 
                                     style="position: absolute; left: 0; z-index: 50; margin-top: 0.25rem;  border-radius: 0.375rem; box-shadow: 0 10px 15px -3px rgba(0, 0, 0, 0.1); max-height: 15rem; overflow-y: auto; width: 100%;" 
        ...
        <table style="width: 100%; border-collapse: collapse; text-align: left;">
            <thead style="position: sticky; top: 0; z-index: 10; box-shadow: 0 1px 2px 0 rgba(0, 0, 0, 0.05);">
                <tr>
                    ...
                    <th class="pos-grid-th">Jatuh Tempo GR</th>
                    <th class="pos-grid-th" style="text-align: right;">Total Tagihan</th>
                    <th class="pos-grid-th" style="text-align: right;">Sudah Dibayar</th>
                    <th class="pos-grid-th" style="text-align: right;">Sisa (Nilai Kontrabon)</th>
                </tr>
            </thead>
            <tbody>
                @forelse($unbilled_invoices as $index => $inv)
                    <tr class="{{ $inv['is_selected'] ? 'bg-blue-50 dark:bg-blue-900/20' : '' }}">
                        <td class="pos-grid-td" style="text-align: center;">
                            <input type="checkbox" wire:model.live="unbilled_invoices.{{ $index }}.is_selected" wire:change="toggleInvoice({{ $index }})" style="width: 1.25rem; height: 1.25rem; cursor: pointer; accent-color: #2563eb;">
                        </td>
                        ...
                        <td class="pos-grid-td font-bold" style="text-align: right; color: #2563eb;">{{ number_format($inv['remaining_amount'], 0) }}</td>
                    </tr>
                @empty
                    <tr>
                        <td colspan="7" style="text-align: center; padding: 3rem; color: #6b7280; font-style: italic;" class="dark:text-gray-400">
                            @if($supplier_id)
                                Tidak ada faktur yang belum ditagihkan (dikontrabonkan) untuk pemasok ini.
                            @else
                                Silakan pilih Pemasok terlebih dahulu untuk melihat daftar faktur.
                            @endif
                        </td>
                    </tr>
                @endforelse
    ...
    <div style="flex: none; border-top: 1px solid #e5e7eb; padding: 1rem;" class="pos-dropdown-bg border border-gray-200 dark:border-gray-700">
    ...
    <!-- Bottom Action Bar -->
    <div style="background-color: white; border-top: 1px solid #e5e7eb; padding: 1rem; position: sticky; bottom: 0; z-index: 20;" class="pos-dropdown-bg border border-gray-200 dark:border-gray-700">
        <div class="flex-between">
            <div style="font-size: 0.875rem; color: #6b7280;" class="dark:text-gray-400">
                Memilih {{ collect($unbilled_invoices)->where('is_selected', true)->count() }} dari {{ count($unbilled_invoices) }} faktur.
            </div>
            ...
            <div style="display: flex; gap: 1.5rem; align-items: center;">
                ...
                <button wire:click="save" wire:loading.attr="disabled"
                    ...
                    <span wire:loading.remove wire:target="save">Simpan Kontrabon</span>
